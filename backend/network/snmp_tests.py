...
class SNMPMonitoringTests(APITestCase):
    ...
    def test_collects_and_persists_supported_metrics(self, mock_get_metric):
        ...
        self.assertEqual(SNMPMetric.objects.get(device=self.device, metric='sysName').value, 'edge-router')
    ...
    def test_authentication_or_agent_failure_is_isolated(self, mock_get_metric):
        result = collect_snmp_metrics(self.device)
    ...
    def test_snmp_configuration_can_be_updated_without_enabling_icmp(self):
        ...
        self.assertTrue(self.configuration.snmp_enabled)
