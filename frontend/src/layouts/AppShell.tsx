...
import type { FaultNotification } from '../notifications/types'
...
const navigation: NavigationItem[] = [
  ...
  { key: 'monitoring', label: 'Monitoring History', description: 'Historical measurements', available: true },
  { key: 'fault-history', label: 'Fault History', description: 'Historical fault events', available: true },
  { key: 'notifications', label: 'Notifications', description: 'Monitoring alerts', available: true },
  { key: 'settings', label: 'Settings', description: 'System configuration', available: false },
]
const icons = { dashboard: LayoutDashboardIcon, devices: ServerIcon, faults: AlertTriangleIcon, monitoring: HistoryIcon, 'fault-history': HistoryIcon, notifications: BellIcon, settings: SettingsIcon }
...
function NotificationIndicator() {
  ...
  useEffect(() => {
    let cancelled = false
    void getNotificationService().list().then((result) => {
  ...
  return <div className="relative">
    <button type="button" className="relative rounded-xl p-2.5 text-slate-500 hover:bg-slate-100" aria-label={unread ? `${unread} unread notifications` : 'Notifications'} aria-expanded={open} onClick={() => setOpen((value) => !value)}>
      <BellIcon className="size-5" />
      {unread > 0 && <span className="absolute right-1.5 top-1.5 grid min-w-4 translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-red-600 px-1 text-[9px] font-bold leading-4 text-white" aria-hidden="true">{unread > 9 ? '9+' : unread}</span>}
    </button>
    {open && <div className="absolute right-0 top-12 z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><div><p className="text-sm font-semibold text-slate-900">Notifications</p><p className="text-xs text-slate-500">{unread} unread</p></div><a href="/notifications" className="text-xs font-semibold text-slate-700 hover:text-slate-950">View all</a></div>
      {error ? <p className="px-4 py-6 text-center text-xs text-slate-500">Notifications are unavailable.</p> : recent.length === 0 ? <p className="px-4 py-6 text-center text-xs text-slate-500">No detected fault notifications.</p> : <div className="divide-y divide-slate-100">{recent.map((notification) => <button key={notification.id} type="button" onClick={() => window.location.replace('/notifications')} className="block w-full px-4 py-3 text-left hover:bg-slate-50"><div className="flex items-start gap-3"><span className={`mt-1.5 size-2 shrink-0 rounded-full ${notification.status === 'unread' ? 'bg-red-500' : 'bg-slate-300'}`} aria-hidden="true" /><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-900">{notification.faultType.replaceAll('_', ' ')}</p><p className="mt-0.5 truncate text-xs text-slate-500">{notification.deviceName} · {notification.severity}</p><p className="mt-1 text-[10px] text-slate-400">{new Date(notification.detectedAt).toLocaleString()}</p></div></div></button>)}</div>}
    </div>}
  </div>
}
...
export function AppShell({ children }: AppShellProps) {
  ...
  const routes: Record<string, string> = { dashboard: '/', devices: '/devices', faults: '/faults', monitoring: '/monitoring-history', 'fault-history': '/fault-history', notifications: '/notifications' }
