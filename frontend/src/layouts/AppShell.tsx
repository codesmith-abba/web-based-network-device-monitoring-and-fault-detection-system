...
const navigation: NavigationItem[] = [
  ...
  { key: 'devices', label: 'Devices', description: 'Manage network devices', available: false },
  { key: 'faults', label: 'Faults', description: 'Active and resolved faults', available: false },
...
const icons = {
  ...
  devices: ServerIcon,
  faults: AlertTriangleIcon,
  monitoring: HistoryIcon,
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-20 items-center justify-between border-b border-slate-200 px-6">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-xl bg-slate-950 text-white shadow-sm">
              <ActivityIcon className="size-5" />
            </div>
            <div>
              <p className="text-sm font-semibold tracking-tight text-slate-950">NetWatch</p>
              <p className="text-xs text-slate-500">Network Monitoring</p>
            </div>
          </div>
          <button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation"><XIcon className="size-5" /></button>
        </div>
          ...
          {navigation.map((item) => {
            ...
            return (
              <button
                ...
                title={item.available ? item.description : `${item.label} will be implemented in a later phase`}
                className={`group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition ${active ? 'bg-slate-950 text-white shadow-sm' : item.available ? 'text-slate-600 hover:bg-slate-100' : 'cursor-not-allowed text-slate-400'}`}
                onClick={() => item.available && setMobileOpen(false)}
              >
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <button className="rounded-xl p-2 text-slate-600 hover:bg-slate-100 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><MenuIcon className="size-5" /></button>
            <div>
              <p className="text-sm font-semibold text-slate-950">Network Operations</p>
              <p className="hidden text-xs text-slate-500 sm:block">Centralized monitoring and fault visibility</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="hidden items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 sm:flex">
