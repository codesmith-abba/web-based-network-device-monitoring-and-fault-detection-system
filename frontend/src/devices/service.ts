...
const store = initialDevices.map((device) => ({ ...device }))
...
function validate(input: DeviceRegistrationInput) {
  ...
  if (!input.monitoring) throw new DeviceServiceError('Monitoring configuration is required.', 'DEVICE_VALIDATION_ERROR')
}
...
const mockService: DeviceService = {
  async list() {
    ...
    if (currentScenario === 'partial') return store.slice(0, 2).map((device) => ({ ...device }))
    return store.map((device) => ({ ...device }))
  },
  async create(input) {
    validate(input)
...
export function getDeviceService(): DeviceService {
  if (import.meta.env.VITE_DEVICES_ADAPTER === 'mock') return mockService
  return {
    async list() { throw new DeviceServiceError('Device management is not connected yet. Configure the Django/DRF device adapter when the backend contract is available.', 'DEVICES_NOT_CONFIGURED') },
    async create() { throw new DeviceServiceError('Device registration is not connected yet. No backend endpoint has been assumed.', 'DEVICES_NOT_CONFIGURED') },
    async update() { throw new DeviceServiceError('Device editing is not connected yet. No backend endpoint has been assumed.', 'DEVICES_NOT_CONFIGURED') },
    async setMonitoring() { throw new DeviceServiceError('Monitoring controls are not connected yet. No backend endpoint has been assumed.', 'DEVICES_NOT_CONFIGURED') },
  }
}
